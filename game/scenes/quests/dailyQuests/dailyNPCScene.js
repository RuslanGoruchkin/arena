...
const dailyNPCScene = new Scene("dailyNPCScene");

dailyNPCScene.enter(ctx => {
...
dailyNPCScene.on("text", ctx => {
    stateWrapper(ctx, (ctx, state) => {
        switch (ctx.update.message.text) {
            case translate(state, "texts.accept"):
                let player = state.player;
                let questsNames = _.map(quests, "name");
                let dailyQuests = questsNames.filter(name => name.match(/^daily/));
                ...
                console.log("Current quest is " + JSON.stringify(player.currentQuest));
                enterScene(ctx, "dailyIntroScene", state);
                break;
