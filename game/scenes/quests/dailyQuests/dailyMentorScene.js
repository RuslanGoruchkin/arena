...
const dailyMentorScene = new Scene("dailyMentorScene");

dailyMentorScene.enter(ctx => {
    stateWrapper(ctx, (ctx, state) => {
        return keyboard(ctx, translate(state, "texts.quests.dailyMentorScene"), [translate(state, "texts.go")], {
            playerId: state.player.id
        });
    });
});

dailyMentorScene.on("text", ctx => {
    stateWrapper(ctx, (ctx, state) => {
        ...
        switch (ctx.update.message.text) {
            case translate(state, "texts.go"):
                ...
                let dailyNPC = _.cloneDeep(gameModules.dailyNPC);
                let room = [
                    [
...
module.exports = dailyMentorScene;
